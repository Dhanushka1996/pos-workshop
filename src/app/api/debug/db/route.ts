...
export async function GET() {
  ...
  const isDirectUrl =
    raw.includes(':5432') && raw.includes('supabase.co') && !raw.includes('pooler');

  return NextResponse.json({
    masked_url: masked,
    is_direct_connection: isDirectUrl,
    hint: isDirectUrl
      ? '❌ This is the DIRECT connection URL (port 5432). Vercel cannot reach it. You must use the POOLER URL (port 6543, host contains "pooler.supabase.com").'
      : raw === '(not set)'
